...
class TikTokDownloader(YtdlpDownloader):
    ...
    async def parse_video_info(self, url: str, **kwargs) -> VideoInfo:
        ...
        try:
            ...
            logger.error(f"解析 TikTok 视频失败: {e}")
            raise Exception(f"解析 TikTok 视频失败: {e}")
        
        return self.raw_to_video_info(raw_info, url, Platform.TIKTOK)
    
    async def download_video(self, url: str, quality: str = "best",
                             format_id: Optional[str] = None,
                             audio_only: bool = False, **kwargs) -> str:
        """下载 TikTok 视频"""
        task_id = f"tt_{os.urandom(4).hex()}"
        ...
        logger.info(f"开始下载 TikTok 视频: {url}")
        ...
        try:
            ...
            if progress and progress.filename:
                ...
                if filepath.exists():
                    logger.info(f"TikTok 视频下载完成: {progress.filename}")
                    return str(filepath)
            ...
            if files:
                logger.info(f"TikTok 视频下载完成: {files[0].name}")
                return str(files[0])
