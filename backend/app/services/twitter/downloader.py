...
class TwitterDownloader(YtdlpDownloader):
    ...
    async def parse_video_info(self, url: str, **kwargs) -> VideoInfo:
        ...
        try:
            ...
            logger.error(f"解析 Twitter 视频失败: {e}")
            raise Exception(f"解析 Twitter 视频失败: {e}")
        
        return self.raw_to_video_info(raw_info, url, Platform.TWITTER)
    
    async def download_video(self, url: str, quality: str = "best",
                             format_id: Optional[str] = None,
                             audio_only: bool = False, **kwargs) -> str:
        """下载 Twitter 视频"""
        task_id = f"tw_{os.urandom(4).hex()}"
        ...
        try:
            ...
            if progress and progress.filename:
                ...
                if filepath.exists():
                    logger.info(f"Twitter 视频下载完成: {progress.filename}")
                    return str(filepath)
            ...
            if files:
                logger.info(f"Twitter 视频下载完成: {files[0].name}")
                return str(files[0])
